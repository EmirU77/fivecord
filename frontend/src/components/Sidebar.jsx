...
export default function Sidebar({
  ...
  return (
    <div className="w-64 bg-[#0f1322]/95 backdrop-blur-2xl flex flex-col shrink-0 select-none border-r border-white/5 z-10 shadow-2xl relative">
      {/* Sleek Synapse Server Header with Dropdown */}
      <div className="relative">
        <div 
          onClick={() => setIsServerMenuOpen(prev => !prev)}
          className="h-13 border-b border-white/5 px-4 flex items-center justify-between hover:bg-white/[0.04] cursor-pointer transition-all duration-200 group"
        ...
        >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <ChevronDown className={`w-4 h-4 text-[#94a3b8] group-hover:text-white transition-transform duration-200 ${isServerMenuOpen ? 'rotate-180 text-white' : 'group-hover:translate-y-0.5'}`} />
          </div>
        </div>
        ...
        {isServerMenuOpen && (
          <div 
            ...
            className="absolute top-14 left-2 right-2 z-40 rounded-2xl bg-[#13172a]/95 backdrop-blur-2xl border border-white/10 shadow-2xl p-1.5 text-xs text-[#e2e8f0] animate-in fade-in slide-in-from-top-1 duration-150"
          >
            <button
              onClick={() => {
                setIsServerMenuOpen(false);
              ...
              className="w-full flex items-center justify-between px-3 py-2 rounded-xl hover:bg-gradient-to-r hover:from-indigo-600 hover:to-purple-600 hover:text-white transition-all cursor-pointer group"
            >
              <span className="font-semibold">Kanal Oluştur</span>
              <Plus className="w-4 h-4 text-[#94a3b8] group-hover:text-white" />
            </button>
            ...
            >
              ...
              <Info className="w-4 h-4 text-[#94a3b8] group-hover:text-white" />
            </button>
        ...
        <div>
          <div className="flex items-center justify-between px-2 mb-1.5 group">
            <div 
              ...
              className="flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-[#64748b] group-hover:text-[#94a3b8] transition-colors cursor-pointer"
            >
              <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${textCollapsed ? '-rotate-90' : ''}`} />
            ...
            <button
              ...
              className="p-1 rounded-lg text-[#64748b] hover:text-white hover:bg-white/10 transition-all cursor-pointer"
              ...
              {textChannels.map(ch => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between w-full px-2.5 py-1.5 text-xs transition-all group cursor-pointer ${
                      isActive 
                    ...
                    title="Sol tıkla: Kanala gir | Sağ tıkla: Kanal ayarları"
                  >
                    <div className="flex items-center gap-2 truncate">
                      <Hash className={`w-4 h-4 shrink-0 transition-colors ${isActive ? 'text-indigo-400 drop-shadow-[0_0_6px_rgba(99,102,241,0.6)]' : 'text-[#64748b] group-hover:text-[#94a3b8]'}`} />
                      <span className="truncate">{ch.name}</span>
                    </div>

                    <button
                      onClick={(e) => handleChannelContextMenu(e, ch)}
                      className="opacity-0 group-hover:opacity-100 p-0.5 rounded-lg hover:bg-white/10 text-[#94a3b8] hover:text-white transition-opacity shrink-0"
        ...
        <div>
          <div className="flex items-center justify-between px-2 mb-1.5 group">
            <div 
              ...
              className="flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-[#64748b] group-hover:text-[#94a3b8] transition-colors cursor-pointer"
            >
              <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${voiceCollapsed ? '-rotate-90' : ''}`} />
              <span>Ses Odaları</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] px-1.5 py-0.2 rounded-md bg-indigo-500/15 border border-indigo-500/30 text-indigo-400 font-mono font-bold">
                60 FPS HQ
              </span>
              <button
                onClick={(e) => {
                ...
                return (
                  <div key={ch.id} className="space-y-1">
                    <div
                      ...
                      onContextMenu={(e) => handleChannelContextMenu(e, ch)}
                      className={`flex items-center justify-between px-2.5 py-1.5 text-xs transition-all cursor-pointer group ${
                        isViewingThis 
                          ? 'bg-gradient-to-r from-emerald-500/20 via-teal-500/10 to-transparent text-white font-bold border-l-2 border-emerald-400 shadow-sm rounded-r-xl rounded-l-md' 
                          : 'text-[#94a3b8] hover:bg-white/[0.04] hover:text-[#f1f5f9] rounded-xl'
                      }`}
                      title="Sol tıkla: Odaya katıl | Sağ tıkla: Oda ayarları"
                    >
                      <div className="flex items-center gap-2 truncate">
                        <Volume2 className={`w-4 h-4 shrink-0 transition-colors ${isInThisVoice ? 'text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.7)]' : 'text-[#64748b] group-hover:text-[#94a3b8]'}`} />
                        <span className="truncate">{ch.name}</span>
                      </div>

                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-[#161a2c] text-[#94a3b8] font-mono border border-white/5">
                          {ch.bitrate}
                        </span>
                        <button
                          onClick={(e) => handleChannelContextMenu(e, ch)}
                          className="opacity-0 group-hover:opacity-100 p-0.5 rounded-lg hover:bg-white/10 text-[#94a3b8] hover:text-white transition-opacity"
      ...
      {/* SYNAPSE FLOATING RIGHT-CLICK CONTEXT MENU */}
      {contextMenu && (
        <div 
          style={{ top: `${contextMenu.y}px`, left: `${contextMenu.x}px` }}
          onClick={(e) => e.stopPropagation()}
          className="fixed z-50 w-52 rounded-2xl bg-[#13172a]/95 backdrop-blur-2xl border border-white/10 shadow-2xl p-1.5 text-xs text-[#e2e8f0] animate-in fade-in zoom-in-95 duration-100 select-none"
        >
          <div className="px-2.5 py-1.5 text-[11px] font-bold text-[#94a3b8] truncate border-b border-white/5 mb-1 flex items-center gap-1.5">
            {contextMenu.channel.type === 'text' ? (
              <Hash className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
            ) : (
              <Volume2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            )}
            <span className="truncate font-semibold">{contextMenu.channel.name}</span>
          </div>

          <button
            onClick={() => handleCopyLink(contextMenu.channel)}
            className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:text-white transition-colors cursor-pointer group text-left"
          >
            <div className="flex items-center gap-2">
              <Copy className="w-3.5 h-3.5 text-[#94a3b8] group-hover:text-white" />
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-xl hover:bg-white/10 hover:text-white transition-colors cursor-pointer text-left group"
          >
            <Edit3 className="w-3.5 h-3.5 text-[#94a3b8] group-hover:text-white" />
          ...
          {contextMenu.channel.id === 'text-genel' ? (
            <div className="px-2.5 py-1.5 text-[10px] text-[#64748b] italic">
              🔒 Ana genel sohbet silinemez
            </div>
          ) : (
            <button
              onClick={() => handleStartDelete(contextMenu.channel)}
              className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-xl text-rose-400 hover:bg-rose-500/20 hover:text-rose-300 transition-colors cursor-pointer font-semibold text-left"
            >
      ...
      {/* SYNAPSE VOICE CONNECTION STATUS PANEL */}
      {currentVoiceChannel && (
        <div className="bg-[#0a0c14]/90 backdrop-blur-xl border-t border-emerald-500/20 p-2.5 space-y-2">
          {/* Status Row */}
          <div className="flex items-center justify-between px-1">
              ...
              <div className="flex items-center justify-between">
                ...
                {onReconnectVoice && (
                  <button
                    ...
                    title="Sesi Yeniden Başlat"
                  >
                    <RotateCw className="w-3 h-3 hover:rotate-180 transition-transform" />
                  </button>
                )}
              </div>
              <div className="text-[11px] text-[#94a3b8] truncate group-hover:text-white transition-colors mt-0.5">
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-rose-500/15 hover:bg-rose-500 text-rose-400 hover:text-white transition-all ml-2 shadow-xs cursor-pointer"
            >
          ...
          <div className="grid grid-cols-2 gap-2 pt-1 border-t border-white/5">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                isScreenSharing 
                  ? 'bg-rose-500 text-white animate-pulse shadow-[0_0_15px_rgba(244,63,94,0.5)]' 
                  : 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white shadow-[0_0_12px_rgba(99,102,241,0.35)] hover:scale-[1.02]'
              }`}
            >
      ...
      {!isAppInstalled && (
        <div className="px-3 py-2 border-t border-white/5 bg-[#0a0c14]/70">
          <button
            onClick={onOpenDownload}
            className="w-full py-2 px-3 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-xs font-bold transition-all shadow-md flex items-center justify-center gap-2 group hover:scale-[1.02] cursor-pointer"
          >
